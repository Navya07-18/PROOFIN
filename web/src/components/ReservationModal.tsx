...
type TxStage = "REVIEW" | "AWAITING_METAMASK" | "PENDING_MONAD" | "CONFIRMED" | "ERROR";
...
export function ReservationModal({ event, isOpen, onClose }: ReservationModalProps) {
  ...
  const [stage, setStage] = useState<TxStage>("REVIEW");
  ...
  const handleConfirmReservation = async () => {
    ...
    setStage("AWAITING_METAMASK");
    setErrorMessage("");

    try {
      const signer = await getSigner();
      if (!signer) {
        throw new Error("Unable to obtain signer from MetaMask extension.");
      }

      // Prompt user manually in MetaMask extension
      const result = await reserveSpotOnChain(event.id, event.depositAmount, signer);

      setStage("PENDING_MONAD");
      ...
      const reservation: ReservationData = {
        ...
        checkInTimeWindow: event.checkInTimeWindow,
      ...
      if (err.code === 4001 || err.code === "ACTION_REJECTED" || err.message?.includes("rejected")) {
        setErrorMessage("Transaction was cancelled or rejected in MetaMask.");
      } else {
        setErrorMessage(
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-base font-bold text-charcoal-900">
                Confirm Spot & Lock Deposit
              </h2>
              <p className="text-xs text-charcoal-500">MetaMask Signature & On-Chain Deposit</p>
            </div>
          </div>
          <button
          ...
          {stage === "CONFIRMED" ? (
            ...
            <div className="text-center py-4 space-y-4">
              ...
              <div>
                ...
                <p className="text-xs text-charcoal-600 max-w-sm mx-auto mt-1 leading-relaxed">
                  Your deposit of <strong>{event.depositAmount} MON</strong> is locked in the smart contract. Check in between <strong>{event.checkInTimeWindow}</strong> to get your MON returned automatically.
                </p>
              </div>
              ...
              {txHash && (
                <div className="bg-charcoal-50 border border-charcoal-200/60 rounded-xl p-3 text-left">
                  <div className="flex items-center justify-between text-xs">
                    <span className="text-charcoal-500 font-medium">Monad Testnet Transaction</span>
                    <a
            ...
            /* Awaiting User Manual Action in MetaMask */
            <div className="text-center py-8 space-y-4">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-charcoal-600 max-w-xs mx-auto leading-relaxed">
                  Please open your MetaMask popup window and click <strong>Confirm</strong> to deposit <strong>{event.depositAmount} MON</strong> and reserve your spot.
                </p>
              </div>
              <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-xl bg-charcoal-50 border border-charcoal-200 text-xs font-mono text-charcoal-600">
                ...
                <span>Waiting for manual signature...</span>
              </div>
            </div>
          ) : stage === "PENDING_MONAD" ? (
            /* Pending Monad Block Finality */
            <div className="text-center py-8 space-y-4">
              ...
              <div>
                <h3 className="text-lg font-bold text-charcoal-900">
                  Broadcasting Commitment to Monad...
                </h3>
                <p className="text-xs text-charcoal-500 mt-1 max-w-xs mx-auto">
                  Locking {event.depositAmount} MON in the smart contract. Finality &lt; 800ms.
                </p>
              </div>
            </div>
          ) : (
            ...
            <>
              ...
              <div className="bg-charcoal-50/80 rounded-2xl p-4 border border-charcoal-200/50 space-y-3">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <h4 className="font-bold text-sm text-charcoal-900 mt-1">{event.title}</h4>
                ...
                <div className="pt-2.5 border-t border-charcoal-200/40 grid grid-cols-2 gap-2 text-xs">
                  <div>
                    <span className="text-charcoal-400 block text-[10px]">Reservation Timing</span>
                    <span className="font-medium text-charcoal-800 flex items-center gap-1 mt-0.5">
                      <Calendar className="w-3 h-3 text-indigo-500" />
                      {event.eventDate} ({event.eventTimeRange})
                    </span>
                  </div>
                  <div>
              ...
              <div className="bg-amber-50/80 border border-amber-200/80 rounded-2xl p-3.5 flex items-start gap-2.5">
                ...
                <div className="text-xs text-amber-900">
                  <p className="font-bold">Strict Check-In Timing Rule</p>
                  <p className="mt-0.5 text-amber-800/90 leading-relaxed">
                    Check-in is allowed only between <strong>{event.checkInTimeWindow}</strong>. Show up during this window to claim your spot and get 100% of your <strong>{event.depositAmount} MON</strong> back. Failure to check in forfeits the deposit.
                  </p>
                </div>
              </div>
              ...
              >
                ...
                <span>
                  {!account
                    ...
                    : `Confirm & Sign in MetaMask (${event.depositAmount} MON)`}
                </span>
